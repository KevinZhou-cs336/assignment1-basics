...
class SwiGLUFeedForwardNetwork(torch.nn.Module):
    ...
    Applied identically and independently to every token position:
        W_1, W_3  up-project  d_model → d_ff  (gate stream and value stream)
        SiLU(z) = z · σ(z)    gates the W_1 branch element-wise
        ⊙         element-wise product between SiLU(W_1 x) and W_3 x
        W_2       down-project d_ff → d_model

    State dict keys:
        ffn.w1.weight  (d_ff, d_model)   gate up-projection
        ffn.w2.weight  (d_model, d_ff)   down-projection
        ffn.w3.weight  (d_ff, d_model)   value up-projection
    """
    ...
    def __init__(
        ...
        self.d_ff = d_ff
        # w1, w3: up-projections; weight shape (d_ff, d_model)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Step 1: Gate stream — W_1 x, then apply SiLU(z) = z * σ(z)
        # einsum "ji,...i→...j": w1.weight (d_ff, d_model) × x (..., d_model) → (..., d_ff)
        w1_x = torch.einsum("ji,...i ->...j", self.w1.weight, x)
        # SiLU element-wise via "...i,...i→...i" (equivalent to w1_x * sigmoid(w1_x))
        # silu_w1x: (..., d_ff)
        ...
        # "...i,...i→...i": Hadamard product along d_ff
